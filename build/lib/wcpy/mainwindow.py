...
from astropy import __version__ as version_astropy
# specutils
from specutils import Spectrum1D
from specutils import __version__ as version_specutils
# drpy
...
from .utils import plotSpectrum, loadLineList, loadSpectrum, saveSpectrum
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def on_save_triggered(self):
        ...
        saveFileDialog = CheckBoxFileDialog(checkBox_text='Enhanced CSV Files (*.ecsv)')
        saveFileDialog.setAcceptMode(QtWidgets.QFileDialog.AcceptSave)
        saveFileDialog.setNameFilters(['FITS (*.fits *.fit *.FITS *.FIT)'])
        saveFileDialog.selectFile(os.path.splitext(self.path_to_file)[0])

        if saveFileDialog.exec_() == QtWidgets.QDialog.Accepted:

            # ``path_to_file``
            path_to_file = saveFileDialog.selectedUrls()[0].toLocalFile()
            ...
            saveECSV = saveFileDialog.checkBox.checkState() == QtCore.Qt.Checked

            saveSpectrum(
                path_to_file=path_to_file, spectrum=self.spectrum, 
                peak_table=self.peak_table, saveECSV=saveECSV)
            
            self.statusBar.showMessage(f'{path_to_file} saved.', 2000)
    ...
    def on_about_triggered(self):
        ...
        text = (
            "Wavelength Calibrator (ver "
            f"<a href='https://pypi.org/project/astro-drpy/{version}'>{version}</a> "
            'released on 2023-08-04) '
            'is a graphical user interface to facilitate '
            'wavelength calibration. It is developed by '
            f"<a href='mailto: rnzhao@nao.cas.cn'>Ruining Zhao</a> "
            'at National Astronomical Observatories, Chinese Academy of Sciences. '
            'Report issue through '
            "<a href='https://github.com/RuiningZHAO/wcpy/issues'>GitHub</a>.<br>"
            '<br>'
            f'numpy: {version_numpy}<br>'
            f'scipy: {version_scipy}<br>'
            f'matplotlib: {version_matplotlib}<br>'
            f'astropy: {version_astropy}<br>'
            f'specutils: {version_specutils}<br>'
            f'astro-drpy: {version_drpy}<br>'
            f'PyQt5: {QtCore.QT_VERSION_STR}<br>'
        )
    ...
    def on_fit_clicked(self):
        ...
        if method == 'poly':
            meta_peak['DEGREE'] = (degree, 'parameter of fitting method')
        elif method == 'spline3':
            meta_peak['PIECE'] = (n_piece, 'parameter of fitting method')
